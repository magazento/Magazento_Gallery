...
class Magazento_Gallery_Block_Admin_Item_Edit_Tab_Formvimeo extends Mage_Adminhtml_Block_Widget_Form {


    protected function _prepareForm() {
        $model = Mage::registry('gallery_item');
        $model->setData('item_type','vimeo');
        
        $form = new Varien_Data_Form(array('id' => 'edit_form_item', 'action' => $this->getData('action'), 'method' => 'post'));
        $form->setHtmlIdPrefix('item_');
        $fieldset = $form->addFieldset('base_fieldset', array('legend' => Mage::helper('gallery')->__('Vimeo Information'), 'class' => 'fieldset-wide'));
        if ($model->getItemId()) {
        ...
        if ($model->getData('item_address')) {
            $vimeolink = Mage::helper('gallery')->getItemVimeo($model,100,150); 
        }
        ...
        $fieldset->addField('item_address', 'text', array(
            'name' => 'item_address',
            'label' => Mage::helper('gallery')->__('Vimeo video ID'),
            'title' => Mage::helper('gallery')->__('Vimeo video ID'),
            'required' => true,
            'note' => 'Example:<br/> 
                       http://vimeo.com/43922070 <br/>
                       Video ID - <u>43922070</u> <br/>'. $vimeolink         
        ));        
