...
class Magazento_Gallery_Block_Admin_Item_Edit_Tab_Formimage extends Mage_Adminhtml_Block_Widget_Form {


    protected function _prepareForm() {
        $model = Mage::registry('gallery_item');
        $model->setData('item_type','image');
        
        $path = Mage::helper('gallery')->getImageFileHttp(); 
        $model->setData('image',$path.DS.$model->getData('item_address'));
        
        $image = '';
        if ($model->getData('item_address')) {
           $path = Mage::helper('gallery')->getImageFileHttp(); 
           $image = '<img width="150" height="100" src="'.$path.DS.$model->getData('item_address').'">';
        }        
        
        $form = new Varien_Data_Form(array('id' => 'edit_form_item', 'action' => $this->getData('action'), 'method' => 'post'));
        $form->setHtmlIdPrefix('item_');
        $fieldset = $form->addFieldset('base_fieldset', array('legend' => Mage::helper('gallery')->__('Image Information'), 'class' => 'fieldset-wide'));
        if ($model->getItemId()) {
