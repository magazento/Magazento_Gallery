...
class Magazento_Gallery_Block_Admin_Item_Edit_Tab_Formyoutube extends Mage_Adminhtml_Block_Widget_Form {


    protected function _prepareForm() {
        $model = Mage::registry('gallery_item');
        $model->setData('item_type','youtube');
        
        if ($model->getData('item_address')) {
            $youtubelink = Mage::helper('gallery')->getItemYoutube($model,100,150); 
        }
        
        $form = new Varien_Data_Form(array('id' => 'edit_form_item', 'action' => $this->getData('action'), 'method' => 'post'));
        $form->setHtmlIdPrefix('item_');
        $fieldset = $form->addFieldset('base_fieldset', array('legend' => Mage::helper('gallery')->__('YouTube Information'), 'class' => 'fieldset-wide'));
        if ($model->getItemId()) {
        ...
        $fieldset->addField('item_address', 'text', array(
            'name' => 'item_address',
            'label' => Mage::helper('gallery')->__('Youtube video ID'),
            'title' => Mage::helper('gallery')->__('Youtube video ID'),
            'required' => true,
            'note' => 'Example:<br/> 
                       http://www.youtube.com/watch?v=w7NI_PZ8sj4  <br/>
                       Video ID - <u>w7NI_PZ8sj4</u> <br/>'. $youtubelink
        ));        
